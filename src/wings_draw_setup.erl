...
work(Vab, #dlo_src{}=Src, St) ->
    work(Vab, Src, St, undefined).

work(Vab, #dlo_src{we=#we{fs=Ftab}}=D, St, Attr) ->
    Update = case Vab of
                 none -> true;
                 #vab{face_vs=none} -> true;
                 #vab{face_fn=none} -> true;
                 #vab{} -> false
             end,
    case Update of
        true ->
...
smooth(Vab, #dlo_src{}=Src, St) ->
    smooth(Vab, Src, St, undefined).

smooth(Vab0, Src, St, Attr) ->
    Vab = case Vab0 of
              none ->
                  work(Vab0, Src, St, Attr);
              #vab{face_vs=none} ->
                  work(Vab0, Src, St, Attr);
              #vab{} ->
                  Vab0
...
plain_flat_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo) ->
...
uv_flat_faces([{Mat,Fs}|T], Src, Start0, Vs0, Fmap0, MatInfo0) ->
    {Start,Vs,FaceMap} = uv_flat_faces_1(Fs, Src, Start0, Vs0, Fmap0),
    MatInfo = [{Mat,?GL_TRIANGLES,Start0,Start-Start0}|MatInfo0],
    uv_flat_faces(T, Src, Start, Vs, FaceMap, MatInfo);
uv_flat_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo) ->
    FaceMap = array:from_orddict(sort(FaceMap0)),
    create_vab([vertices,face_normals,uvs], Vs, FaceMap, MatInfo).

uv_flat_faces_1([{Face,Edge}|Fs], #dlo_src{we=We,ns=Ns}=Src, Start, Vs, FaceMap) ->
    UVs = wings_va:face_attr(uv, Face, Edge, We),
    case array:get(Face, Ns) of
	[Normal|Pos =[_,_,_]] ->
	    uv_flat_faces_1(Fs, Src, Start+3,
			    add_tri(Vs, Normal, Pos, UVs),
			    [{Face,{Start,3}}|FaceMap]);
	[Normal|Pos] ->
	    uv_flat_faces_1(Fs, Src, Start+6,
	...
	{Normal,Faces,VsPos} ->
	    ...
	    uv_flat_faces_1(Fs, Src, NoVs+Start,
			    VsBin, [{Face,{Start,NoVs}}|FaceMap])
    end;
uv_flat_faces_1([], #dlo_src{}, Start, Vs, FaceMap) ->
    {Start,Vs,FaceMap}.

%% Also needs uv's
tangent_flat_faces([{Mat,Fs}|T], Src, Start0, Vs0, Fmap0, MatInfo0, Ts0) ->
    {Start,Vs,FaceMap,Ts} = tangent_flat_faces_1(Fs, Src, Start0, Vs0, Fmap0, Ts0),
    MatInfo = [{Mat,?GL_TRIANGLES,Start0,Start-Start0}|MatInfo0],
    tangent_flat_faces(T, Src, Start, Vs, FaceMap, MatInfo, Ts);
tangent_flat_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo, {VsTs0, RevF2V}) ->
    FaceMap = array:from_orddict(sort(FaceMap0)),
...
tangent_flat_faces_1([{Face,Edge}|Fs], #dlo_src{we=We,ns=Ns}=Src, Start, Vs, FaceMap, Ts0) ->
    UVs = wings_va:face_attr(uv, Face, Edge, We),
    case array:get(Face, Ns) of
	[Normal|Pos =[_,_,_]] ->
	    tangent_flat_faces_1(Fs, Src, Start+3,
				 add_tri(Vs, Normal, Pos, UVs),
				 [{Face,{Start,3}}|FaceMap],
	...
	[Normal|Pos] ->
	    tangent_flat_faces_1(Fs, Src, Start+6,
				 add_quad(Vs, Normal, Pos, UVs),
				 [{Face,{Start,6}}|FaceMap],
				 add_ts(Pos, UVs, Normal,
					wings_face:vertices_ccw(Face, We), Ts0));
	{Normal,Faces,VsPos}=Info ->
	    NoVs  = length(Faces) * 3,
	    VsBin = add_poly(Vs, Normal, Faces,
			     list_to_tuple(VsPos), list_to_tuple(UVs)),
	    tangent_flat_faces_1(Fs, Src, NoVs+Start,
				 VsBin, [{Face,{Start,NoVs}}|FaceMap],
				 add_ts(Info, UVs, Normal,
					wings_face:vertices_ccw(Face, We), Ts0))
    end;
tangent_flat_faces_1([], #dlo_src{}, Start, Vs, FaceMap, Ts) ->
    {Start,Vs,FaceMap,Ts}.


col_flat_faces([{Mat,Fs}|T], Src, Start0, Vs0, Fmap0, MatInfo0) ->
    {Start,Vs,FaceMap} = col_flat_faces_1(Fs, Src, Start0, Vs0, Fmap0),
    MatInfo = [{Mat,?GL_TRIANGLES,Start0,Start-Start0}|MatInfo0],
    col_flat_faces(T, Src, Start, Vs, FaceMap, MatInfo);
col_flat_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo) ->
    FaceMap = array:from_orddict(sort(FaceMap0)),
    create_vab([vertices,face_normals,colors], Vs, FaceMap, MatInfo).

col_flat_faces_1([{Face,Edge}|T], #dlo_src{we=We,ns=Ns}=Src, Start, Vs0, Fmap0) ->
    Cols = wings_va:face_attr(color, Face, Edge, We),
    case array:get(Face, Ns) of
	[Normal|Pos =[_,_,_]] ->
	    Vs = add_col_tri(Vs0, Normal, Pos, Cols),
	    Fmap = [{Face,{Start,3}}|Fmap0],
	    col_flat_faces_1(T, Src, Start+3, Vs, Fmap);
	[Normal|Pos] ->
	    Vs = add_col_quad(Vs0, Normal, Pos, Cols),
	    Fmap = [{Face,{Start,6}}|Fmap0],
	    col_flat_faces_1(T, Src, Start+6, Vs, Fmap);
	{Normal,Faces,VsPos} ->
	    NumVs  = length(Faces) * 3,
	    Vs = add_col_poly(Vs0, Normal, Faces,
			      list_to_tuple(VsPos), list_to_tuple(Cols)),
	    Fmap = [{Face,{Start,NumVs}}|Fmap0],
	    col_flat_faces_1(T, Src, Start+NumVs, Vs, Fmap)
    end;
col_flat_faces_1([], #dlo_src{}, Start, Vs, Fmap) ->
    {Start,Vs,Fmap}.
...
col_uv_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo) ->
    FaceMap = array:from_orddict(sort(FaceMap0)),
    create_vab([vertices,face_normals,colors,uvs],
               Vs, FaceMap, MatInfo).

col_uv_faces_1([{Face,Edge}|Fs], #dlo_src{we=We,ns=Ns}=Src, Start, Vs, FaceMap) ->
    UVs = wings_va:face_attr([color|uv], Face, Edge, We),
    case array:get(Face, Ns) of
	[Normal|Pos =[_,_,_]] ->
	    col_uv_faces_1(Fs, Src, Start+3,
			   add_col_uv_tri(Vs, Normal, Pos, UVs),
			   [{Face,{Start,3}}|FaceMap]);
	[Normal|Pos] ->
	    col_uv_faces_1(Fs, Src, Start+6,
	...
	{Normal,Faces,VsPos} ->
	    ...
	    VsBin = add_col_uv_poly(Vs, Normal, Faces,
			     list_to_tuple(VsPos), list_to_tuple(UVs)),
	    col_uv_faces_1(Fs, Src, NoVs+Start,
			   VsBin, [{Face,{Start,NoVs}}|FaceMap])
    end;
col_uv_faces_1([], #dlo_src{}, Start, Vs, FaceMap) ->
    {Start,Vs,FaceMap}.
...
col_tangent_faces([], #dlo_src{}, _Start, Vs, FaceMap0, MatInfo, {VsTs0, RevF2V}) ->
    FaceMap = array:from_orddict(sort(FaceMap0)),
    VsTs = array:map(fun(_V, {T,BT}) ->
			     {e3d_vec:norm(T),e3d_vec:norm(BT)}
		     end, VsTs0),
...
col_tangent_faces_1([{Face,Edge}|Fs], #dlo_src{we=We,ns=Ns}=Src,
                    Start, Vs, FaceMap, Ts0) ->
    UVs = wings_va:face_attr([color|uv], Face, Edge, We),
    case array:get(Face, Ns) of
	[Normal|Pos =[_,_,_]] ->
	    col_tangent_faces_1(Fs, Src, Start+3,
				add_col_uv_tri(Vs, Normal, Pos, UVs),
				[{Face,{Start,3}}|FaceMap],
				add_ts(Pos, [UV|| [_|UV] <- UVs], Normal,
				       wings_face:vertices_ccw(Face, We), Ts0));
	[Normal|Pos] ->
	    col_tangent_faces_1(Fs, Src, Start+6,
				add_col_uv_quad(Vs, Normal, Pos, UVs),
				[{Face,{Start,6}}|FaceMap],
				add_ts(Pos, [UV|| [_|UV] <- UVs], Normal,
				       wings_face:vertices_ccw(Face, We), Ts0));
	Info = {Normal,Faces,VsPos} ->
	    ...
	    VsBin = add_col_uv_poly(Vs, Normal, Faces,
				    list_to_tuple(VsPos), list_to_tuple(UVs)),
	    col_tangent_faces_1(Fs, Src, NoVs+Start,
				VsBin, [{Face,{Start,NoVs}}|FaceMap],
				add_ts(Info, [UV|| [_|UV] <- UVs], Normal,
				       wings_face:vertices_ccw(Face, We), Ts0))
    end;
col_tangent_faces_1([], #dlo_src{}, Start, Vs, FaceMap,Ts) ->
    {Start,Vs,FaceMap,Ts}.


setup_smooth_normals(#vab{face_map=Fmap0}=Vab,
                     #dlo_src{we=#we{}=We,ns=Ns0,mirror=MM}) ->
