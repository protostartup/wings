...
init() ->
    ...
    wings_pref:set_default(tweak_draw, true),
...
%% this is the exiting event from wings_pick after paint_pick/3
    wings_wm:later({new_state,St}),
    pop;
handle_initial_event(#mousemotion{}, _, St, #tweak{clk={one,_}}) ->
    wings_wm:later({new_state,St}),
    pop;
...
end_drag(Mode, #dlo{src_sel={_,_},src_we=#we{id=Id}}=D0, #st{shapes=Shs0}=St0) ->
    case Mode of
      slide ->
        case wings_io:is_key_pressed($f) of
          false ->
...
toggle_draw(Bool) ->
    wings_pref:set_value(tweak_draw, Bool).
...
tweak_info_line() ->
    ...
    M3 = [Rmb,?__(4,"Menu")],
    M4 = case wings_pref:get_value(camera_mode) of
...
tweak_keys_info() ->
    ...
    case wings_pref:get_value(tweak_draw) of
      true when This ->
        Cam = wings_pref:get_value(camera_mode),
        case wings_pref:get_value(tweak_active) of
          true ->
...
statusbar() ->
    ...
    Draw = wings_pref:get_value(tweak_draw),
