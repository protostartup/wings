...
-record(drag,
	{x,y,					% Original 2D position,
	 ...
	 psum=[0,0,0,0,0],		% Whereas xs,ys,zs,p4 and p5 are the displayed distances
...
%% from the Stop time (relased) and if the result is less than 300000 ms, then
...
%% When Lmb timer is less than 300000 ms, we Accept and finish the drag, if
%% not, then we continue usibng the lmb parameter.
    Stop = now(),
    Time = timer:now_diff(Stop, StartTime),
    % io:format("Time ~p\n",[Time]),
    case Time < 300000 of
