...
redraw(Info, St) ->
	...
	fun() ->
		...
		call_post_hook(St),
		TweakAxes = wings_tweak:statusbar(),
		case Info =/= [] andalso wings_wm:get_prop(show_info_text) of
		    true when is_list(TweakAxes) ->
		        wings_io:info(0,0,[TweakAxes,"\n",Info]);
		    false when is_list(TweakAxes) ->
		        wings_io:info(0,0,TweakAxes);
		    true ->
		        wings_io:info(0,0,Info);
		    false -> ok
		end,
...
tools_menu(_) ->
    ...
    {?__(22,"Freeze"),freeze,
     ...
     separator,
     {?__(37,"Tweak"),tweak_menu,?__(38,"Open the Tweak menu")}].
...
menu_toolbar_action({menu_toolbar, {B, OrigXY, Side}}, #st{selmode=Mode}=St)
  when B =:= 4; B =:= 5 ->
    Cmd = case {Side, B} of
      {history,4} -> {edit,redo};
      {history,5} -> {edit,undo};
      {edge,4} when Mode =:= edge -> {select,{edge_loop,next_edge_loop}};
      {edge,5} when Mode =:= edge -> {select,{edge_loop,prev_edge_loop}};
      {_,4} -> {select,more};
      {_,5} -> {select,less}
    end,
    wings_wm:send_after_redraw(geom, {menu_toolbar,OrigXY}),
...
menu_toolbar_action({menu_toolbar,{B,OrigXY,Side}}, #st{sel=Sel}=St0) ->
    ...
    {Cmd, St, Kill} = case {Side, Sel} of
        ...
        {deselect, []} ->
            {{S,all},St0,false};
        {edge,_} when B =:= 3 ->
            {{S,{edge_loop,edge_loop}}, St0, false};
        {edge,_} when B =:= 2 ->
            {{S,{edge_loop,edge_ring}}, St0, false};
        {face,_} when B =:= 3 ->
            {{S,{edge_loop,edge_loop_to_region}}, St0, false};
        {history,_} when B =:= 1 ->
            {{edit,undo}, St0, false};
        {history,_} when B =:= 3 ->
            {{edit,redo}, St0, false};
        ...
        {SelMode, []} ->
            {{S,SelMode}, St0#st{selmode=SelMode}, true};
        {SelMode,_} ->
            {{S,SelMode}, St0, false}
    end,
